    ...
    def handle(self, *args, **kwargs):
        ...
        if not tags or not features:
            self.stdout.write(self.style.ERROR(
                "You must create tags and features before trying to create banners"))
            self.stdout.write(self.style.WARNING(
                "First run |seed_tags --number number_of_tags| and |seed_features --number number_of_features|"))
            return
        ...
        for _ in range(1, number + 1):
            ...
            tags_used = BannerTagFeature.objects.filter(
                feature=feature).values_list('tag', flat=True)
            ...
            if tags_limit == 0:
                self.stdout.write(self.style.WARNING(
                    f"For the current feature '{feature}', there are no available tags. Skipping banner creation."))
                continue
            
            #Selecting a random number of tags to the banner from the list of available tags
            selected_tags = random.sample(
                list(list_of_tags_available), random.randint(1, tags_limit))
            #Defining data
            content = {
                'title': fake.sentence(),
                'text': fake.text(),
            ...
            try:
                ...
                BannerTagFeature.objects.bulk_create(
                    [BannerTagFeature(
                    banner=banner, 
                    feature=feature, 
                    tag=tag) for tag in selected_tags
                    ]
                )
